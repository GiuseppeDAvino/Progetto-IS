...
public class AggiungiPeriferica extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(nome.length() == 0) {
			request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo nome è vuoto");
			...
			if(nome.length() > 40) {
				request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo nome ha una lunghezza maggiore a 40");
				...
				if(tipo.length() == 0) {
					request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo tipo è vuoto");
					session.setAttribute("error-type", "nome");
					session.setAttribute("error", "Campo vuoto");
					response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/user.jsp"));
				}
					...
					if(tipo.length() > 30) {
						request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo tipo ha una lunghezza maggiore a 30");
						session.setAttribute("error-type", "tipoGenerico");
						session.setAttribute("error", "Lunghezza errata");
						response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/user.jsp"));
					}
						...
						if(quantita.length() == 0) {
							request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo quantità è vuoto");
							session.setAttribute("error-type", "nome");
							session.setAttribute("error", "Campo vuoto");
							response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/user.jsp"));
						}
							...
							if(!Validatore.validaQuantita(quantita)) {
								request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo quantità non rispetta il formato");
								...
								if(prezzo.length() == 0) {
									request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo prezzo è vuoto");
									session.setAttribute("error-type", "nome");
									session.setAttribute("error", "Campo vuoto");
									response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/user.jsp"));
								}
									...
									if(!Validatore.validaPrezzo(prezzo)) {
										request.setAttribute("errorTest","L'aggiunta della periferica non va a buon fine poichè il campo prezzo non rispetta il formato");
										session.setAttribute("error-type", "prezzo");
										session.setAttribute("error", "Formato errato");
										response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/user.jsp"));
									}
